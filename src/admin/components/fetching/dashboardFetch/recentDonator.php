<?php
$query = "SELECT u.usr_fname, u.usr_lname, e.ewst_name, r.stg_dateAdd FROM tbl_storage r INNER JOIN tbl_user u ON r.stg_userId = u.usr_id INNER JOIN tbl_ewst e ON r.stg_ewstId = e.ewst_id WHERE u.usr_typ != 1 ORDER BY r.stg_dateAdd DESC LIMIT 10";
$result = $conn->query($query);
$recentDonators = [];
if ($result) {
    while ($row = $result->fetch_assoc()) {
        // Calculate human-readable time difference
        $rcntdnt_time = new DateTime($row['stg_dateAdd']);
