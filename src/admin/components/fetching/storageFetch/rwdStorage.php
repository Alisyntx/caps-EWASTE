...
$item_query = $conn->query("SELECT DISTINCT rwd_stg_item FROM tbl_rwd_storage");
?>
...
<div class="filter-buttons flex justify-start mb-5 bg-bgbox mt-2 p-2 rounded-md border items-center border-bgborder border-opacity-50">
    ...
    <?php while ($item_data = mysqli_fetch_array($item_query)) { ?>
        <button class="filter-btn btn font-popin font-normal shadow-md text-xs btn-xs mx-2" data-item="<?php echo $item_data['rwd_stg_item']; ?>">
            <?php echo $item_data['rwd_stg_item']; ?>
        </button>
    <?php } ?>
</div>
...
<div class="category-box mt-2 mb-5 bg-bgbox border border-bgcard border-1 w-full h-auto rounded-md flex flex-col shadow-md divide-y divide-bgborder">
    <div class="overflow-x-auto">
        <table id="dataTbl" class="table table-xs">
            ...
            <tbody id="itemList">
                <?php
                $item_query = "SELECT * FROM tbl_rwd_storage";
                $result = $conn->query($item_query);
                if ($result->num_rows > 0) {
                    while ($items = $result->fetch_assoc()) {
                        $formatted_date = date('Y-m-d', strtotime($items['rwd_stg_dateAdd']));
                ?>
                        <tr class="item-row" data-item="<?php echo $items['rwd_stg_item']; ?>">
                            <td>
                                <div class="font-semibold font-popin"><?php echo $items['rwd_stg_refnum']; ?></div>
                            </td>
                            <td>
                                <div class="font-semibold font-popin"><?php echo $items['rwd_stg_username']; ?></div>
                            </td>
                            <td>
                                <div class="">
                                    <div class="font-semibold font-popin"><?php echo $items['rwd_stg_user']; ?></div>
                                </div>
                            </td>
                            <td class="">
                                <div class="font-semibold font-popin"><?php echo $items['rwd_stg_item']; ?></div>
                            </td>
                            <td class="">
                                <div class="font-semibold font-popin"><?php echo $items['rwd_stg_points']; ?> points</div>
                            </td>
                            <td class="">
                                <div class="font-semibold font-popin"><?php echo $formatted_date ?></div>
                            </td>
                        </tr>
                <?php
                    }
