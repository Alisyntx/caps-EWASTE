...
$id = $_POST['getId'];
$queryItemName = $conn->query("SELECT * FROM tbl_ewst WHERE ewst_id = '$id'");
...
<div class="modal-box border border-bgborder bg-bgbox">
    ...
    <form class="w-full flex flex-col justify-center items-center" id="delItemForm">
        <i data-lucide="triangle-alert" class="w-10 h-10 text-warning"></i>
        <button class="btn btn-xs outline bg-transparent outline-1 rounded-md outline-bgborder text-bgtext text-xs font-popin"><?php echo $ItemName['ewst_name'] ?></button>
