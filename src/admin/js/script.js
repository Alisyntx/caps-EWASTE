$(document).ready(function () {
        ...
        $.post(
            ...
            function (response) {
                ...
                `;
                    ...
                    var newCategoryHTML = `
                    <div class="animate__animated animate__backInRight mt-2 mb-5 bg-bgbox border border-bgborder w-full h-auto rounded-md  flex flex-col gap-1 shadow-md divide-y divide-bgborder">
                        <div class="p-1 flex justify-between">
                            <button class="btn btn-xs outline bg-transparent outline-1 rounded-md outline-bgborder bg-bg text-bgtext text-xs font-popin">${response.cty_name}
                            </button>
                            <div>
                                <button class="btn btn-xs btn-circle bg-mainbg shadow-md font-poppin bg-transparent text-center text-bgtext text-xs font-popin delCty" id="${response.cty_id}" onclick="delCty.showModal()"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                                <button class="btn btn-xs btn-circle bg-mainbg shadow-md font-poppin bg-transparent text-center text-bgtext text-xs font-popin addItemCtyId" id="${response.cty_id}" onclick="addItemCty.showModal()"><i data-lucide="list-plus" class="w-4 h-4"></i></button>
                            </div>
                        </div>
                        <div class=" ">
    ...
    $("#searchInput, #category, #info, #condition, #points").on(
        "input change",
        function () {
            checkInputs();
            updatePreview();
